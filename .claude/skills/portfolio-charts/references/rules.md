...
- **Stat tile**: label (sentence case, no colon), value (`text-3xl md:text-4xl font-medium tracking-tight`, proportional figures), and an optional delta against a named period ("↑ 12% vs last week"). A good delta uses `text-accent-foreground`. Whether "up" is good depends on the metric (resting heart rate going up is not good).
